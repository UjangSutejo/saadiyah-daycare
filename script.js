...
// ===== HEADER SCROLL EFFECT (Dynamic Opal Style) =====
const header = document.querySelector('.header');

window.addEventListener('scroll', () => {
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;

    // STAGE 2: Medium Shrink (100px - 600px)
    if (scrollTop > 100 && scrollTop <= 600) {
        header.classList.add('scrolled-medium');
        header.classList.remove('scrolled-small'); // Ensure we are not small
    }
    // STAGE 3: Small Shrink ( > 600px)
    else if (scrollTop > 600) {
        header.classList.add('scrolled-small');
        header.classList.remove('scrolled-medium'); // Upgrade to small
    }
    // STAGE 1: Original (< 100px)
    else {
        header.classList.remove('scrolled-medium');
        header.classList.remove('scrolled-small');

        // Close menu if open when returning to top
        if (typeof nav !== 'undefined' && nav.classList.contains('active')) {
            nav.classList.remove('active');
            // Reset icon
            const menuBtn = document.querySelector('.mobile-menu-btn');
            if (menuBtn) {
